...
class Merger {
  ...
  _cleanUp_checkoutBranch() {
    ...
    return this.gUtils.checkout(branch);
  }

  _cleanUp_deleteTempBranch() {
    let tempBranch = this.input.tempBranch;

    return this.gUtils.deleteBranch(tempBranch, true);
  }
  ...
  phase2() {
    ...
    return this.utils.phase(2, 'Fetching PR as local branch', () => Promise.resolve().
      then(() => this.gUtils.checkout(branch)).
      then(() => this.gUtils.pull(branch, true)).
      then(() => this.gUtils.createBranch(tempBranch)).
      then(() => this.cleanUper.register([
        this._cleanUp_deleteTempBranch,
        this._cleanUp_checkoutBranch
      ])).
      then(() => this.gUtils.mergePullRequest(url)));
  }
  ...
  phase3() {
    ...
    return this.utils.phase(3, `Merging into '${branch}'`, () => {
      ...
      return this.gUtils.
        ...
        then(() => this.gUtils.checkout(branch)).
        then(() => this.cleanUper.unregister(this._cleanUp_checkoutBranch)).
        then(() => this.cleanUper.withTask(abortRebase, () => Promise.resolve().
  ...
  phase6() {
    ...
    return this.utils.phase(6, 'Cleaning up and Pushing to origin', () => this.gUtils.
      ...
      then(() => this.cleanUper.unregister(this._cleanUp_deleteTempBranch)).
      then(() => this.utils.
