...
class Merger {
  // Constructor
  constructor(logger, cleanUper, utils, uiUtils, gitUtils, input) {
    this._logger = logger;
  ...
  static getPhases() {
    return [
      ...
      new Phase('5', 'Cleaning untracked files', [
  ...
  phase4() {
    ...
    return this._uiUtils.phase(phase, () => Promise.resolve().
      then(() => this._logger.log(chalk.reset.yellow.bold('    GIT diff:\n'))).
      then(() => this._utils.waitAsPromised(500)).
      then(() => this._gitUtils.diffWithHighlight(`origin/${branch}`)).
      then(() => this._logger.log(chalk.reset.yellow.bold('\n    GIT log:\n'))).
      then(() => this._utils.waitAsPromised(500)).
  ...
  phase6() {
    ...
    return this._uiUtils.phase(phase, () => this._uiUtils.
      ...
      then(() => {
        this._logger.log('    Initializing the CI-checks...\n');
