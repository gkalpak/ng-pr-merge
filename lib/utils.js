...
class Utils {
  ...
  exitWithError(errCode, skipCleanUp) {
    let self = this;
    let errMsg = self.messages.errors[errCode] || errCode || '<no error code>';
    ...
    return function onError(err) {
      ...
      return (!skipCleanUp && self.cleanUper.hasTasks()) ?
          this.offerToCleanUp().then(exit, exit) :
          exit();
    };
  }
  ...
  offerToCleanUp() {
    let doCleanUp = () => this.cleanUper.cleanUp();

    return this.
      askYesOrNoQuestion(chalk.bgYellow.black(this.messages.offerToCleanUp)).
      then(() => this.phase('X', 'Trying to clean up the mess', doCleanUp, true));
  }
