...
class Utils {
  ...
  exitWithError(errOrCode, skipCleanUp) {
    return extraErr => {
      let errMsg = this._messages.errors[errOrCode] || errOrCode || '<no error code>';
  ...
  offerToCleanUp() {
    let doCleanUp = () => {
      let cleanUpPhase = this._messages.cleanUpPhase;
  ...
  phase(phase, doWork, skipCleanUp) {
    ...
    return doWork().
      ...
      catch(this.exitWithError(error || 'ERROR_unexpected', skipCleanUp));
  }
