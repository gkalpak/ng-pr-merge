...
class GitUtils {
  ...
  // Methods - Public
  abortAm() {
    return this.utils.spawnAsPromised('git am --abort');
  }

  abortRebase() {
    return this.utils.spawnAsPromised('git rebase --abort');
  }
  ...
  mergePullRequest(url) {
    ...
    //          To support redirection: this.utils.spawnAsPromised(`curl -L ${url} | git am -3`)
    return new Promise((resolve, reject) => {
      let abortAm = () => this.abortAm();
      let cb = res => this.cleanUper.
        withTask(abortAm, () => this.utils.spawnAsPromised('git am -3', res)).
        then(resolve, reject);
  ...
  setLastCommitMessage(message) {
    ...
    return Promise.resolve().
      then(() => this.utils.writeFileAsPromised(tempFile, message)).
